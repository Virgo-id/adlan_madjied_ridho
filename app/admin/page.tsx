...
// Interface data post untuk basis data karya tulis
interface Post {
  id: string;
...
export default function Home() {
  ...
  // State untuk data karya tulis
  const [daftarKarya, setDaftarKarya] = useState<Post[]>([]);
  const [isLoadingKarya, setIsLoadingKarya] = useState<boolean>(true);
  ...
  // Efek menghitung umur otomatis & tahun copyright
  useEffect(() => {
  ...
  // Efek mengambil data karya tulis dari Supabase
  useEffect(() => {
    async function fetchPosts() {
      try {
        setIsLoadingKarya(true);
        ...
        setIsLoadingKarya(false);
      }
    }
    fetchPosts();
  }, []);

  // Helper untuk mengubah tanggal rilis menjadi teks bersih
  ...
  // Shortcut Alt + L untuk ke halaman Admin
  useEffect(() => {
  ...
  // Observer untuk efek navigasi pagination titik-titik
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans text-zinc-900 overflow-hidden dark:bg-zinc-950 dark:text-zinc-50">
      ...
      <main className="h-screen w-full overflow-y-scroll snap-y snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
        ...
        {/* SLIDE 3: KARYA TULIS (DILEBUR LANGSUNG DI SINI) */}
        <section ref={slideRefs[2]} className="flex h-screen w-full snap-start snap-always flex-col px-6 border-t border-zinc-100 dark:border-zinc-900">
          <div className="mx-auto h-screen w-full max-w-2xl flex flex-col justify-between py-12">
            ...
            {/* GRID KONTEN KARYA TULIS */}
            <div className="grow grid gap-x-6 gap-y-10 grid-cols-1 sm:grid-cols-2 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-zinc-200 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full">
              {isLoadingKarya ? (
                /* SKELETON LOADING */
                [1, 2].map((n) => (
                  <div key={n} className="flex flex-col gap-3 animate-pulse">
                    <div className="w-full aspect-video bg-zinc-200 dark:bg-zinc-800" />
                    <div className="h-4 w-1/4 bg-zinc-200 dark:bg-zinc-800 mt-1" />
                ...
                daftarKarya.map((karya) => {
                  return (
                    ...
                    >
                      ...
                      {/* META DATA: KATEGORI & TANGGAL JADI SATU BARIS */}
                      <div className="flex items-center gap-3 text-xs">
                        ...
                        <span className="text-zinc-500 dark:text-zinc-400">
                          {formatTanggal(karya.created_at)}
                        </span>
                      </div>
                      ...
                      {karya.summary && (
                        <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed line-clamp-2 -mt-1">
                          {karya.summary}
                        </p>
                      )}
                    </Link>
