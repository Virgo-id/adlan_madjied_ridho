...
// Daftarkan properti penerima status sembunyi
...
export default function Sidebar({ hideMobile = false }: SidebarProps) {
  ...
  return (
    <>
      {/* 1. DESKTOP SIDEBAR */}
      <aside className="group fixed left-0 top-0 h-screen w-16 hover:w-56 hidden md:flex flex-col justify-center bg-black border-none z-50 transition-[width] duration-100 ease-in-out">
            ...
            return (
              <Link
                ...
                className={`flex items-center rounded-xl py-3 px-3 text-sm font-bold transition-colors gap-4 ${
                  isSelected ? "bg-emerald-600 text-white" : "text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-100"
                }`}
              >
      ...
      {/* Jika hideMobile bernilai TRUE, kelas 'hidden' akan otomatis melenyapkan bar ini */}
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-black px-4 py-2 border-none ${
        ...
        <div className="flex justify-around items-center max-w-sm mx-auto w-full">
          {menuItems.map((item) => {
            const isSelected = pathname === item.path || (item.path === "/admin/chat" && pathname.startsWith("/admin/chat"));
            return (
              <Link key={item.path} href={item.path} className="flex flex-col items-center gap-1.5 py-1 px-3">
                <span className={`text-sm flex items-center justify-center h-10 w-10 rounded-full transition-all ${
                  isSelected ? "bg-emerald-600 text-white scale-110" : "text-zinc-500"
                }`}>
                  <i className={item.icon} />
                </span>
                <span className={`text-[10px] font-extrabold ${isSelected ? "text-emerald-500" : "text-zinc-500"}`}>
                  {item.name}
                </span>
              </Link>
            );
